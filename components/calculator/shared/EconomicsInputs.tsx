...
// ============================================
// SHIPPING COST INPUT WITH SEK/% TOGGLE
// ============================================
...
// ============================================
// ECONOMICS INPUTS COMPONENT
// ============================================

export interface EconomicsInputsProps {
  /** Aktuella indata */
  inputs: CalculatorInput;
  /** Callback när ett värde ändras */
  onUpdate: <K extends keyof CalculatorInput>(key: K, value: CalculatorInput[K]) => void;
  /** Callback för att rensa ett valfritt värde */
  onClear: (key: keyof CalculatorInput) => void;
  /** Callback för beräkning (optional) */
  onCalculate?: () => void;
  /** Visa beräkna-knapp (default: true) */
  showCalculateButton?: boolean;
  /** Kompakt läge utan branschväljare (för reverse calculator) */
  compact?: boolean;
  /** Visa önskad vinstmarginal (default: true) */
  showProfitMargin?: boolean;
}

/**
 * Delat komponent för ekonomiska indata.
 * Används av både framåt- och bakåt-kalkylatorn.
 */
export function EconomicsInputs({
  inputs,
  onUpdate,
  onClear,
  onCalculate,
  ...
  const desiredMarginWarning = inputs.desiredProfitMargin !== undefined && inputs.desiredProfitMargin > 50
    ? 'Hög vinstmarginal – säkerställ att detta är realistiskt' : undefined;

  return (
    <div className="space-y-6">
      {/* Industry selector - only in non-compact mode */}
      {!compact && (
        <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            <NumericInput
              ...
              suffix="SEK"
              placeholder={inputs.aov > 0 ? `Default: ${Math.round(inputs.aov * (1 - defaults.margin))} SEK` : 'Ange AOV först'}
              min={0}
            />
      ...
      {showCalculateButton && onCalculate && (
        <Button
          ...
          className="w-full"
          onClick={onCalculate}
          disabled={!inputs.aov || inputs.aov <= 0}
        >
          Beräkna
        </Button>
      )}
...
export default EconomicsInputs;
